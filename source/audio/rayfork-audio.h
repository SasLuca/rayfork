...
rf_public void rf_init_audio_device(void);         // Initialize audio device and context
rf_public void rf_close_audio_device(void);        // Close the audio device and context
rf_public bool rf_is_audio_device_ready(void);     // Check if audio device has been initialized successfully
rf_public void rf_set_master_volume(float volume); // Set master volume (listener)
#pragma endregion
