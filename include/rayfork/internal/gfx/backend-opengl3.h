#ifndef RAYFORK_BACKEND_OPENGL3_H
#define RAYFORK_BACKEND_OPENGL3_H
...
{
    rf_int elements_count; // Number of elements in the buffer (QUADS)
    rf_int v_counter;      // Vertex position counter to process (and draw) from full buffer
    rf_int tc_counter;     // Vertex texcoord counter to process (and draw) from full buffer
    rf_int c_counter;      // Vertex color counter to process (and draw) from full buffer
...
typedef struct rf_draw_call
{
    ...
    rf_int vertex_count;     // Number of vertex of the draw
    rf_int vertex_alignment; // Number of vertex required for index alignment (LINES, TRIANGLES)
...
#endif // RAYFORK_BACKEND_OPENGL3_H