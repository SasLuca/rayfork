...
rf_extern rf_render_batch rf_create_custom_render_batch_from_buffers(rf_vertex_buffer* vertex_buffers, rf_int vertex_buffers_count, rf_draw_call* draw_calls, rf_int draw_calls_count)
{
    ...
    for (rf_int i = 0; i < vertex_buffers_count; i++)
    {
        ...
        memset(vertex_buffers[i].texcoords, 0, rf_gdx_texcoord_component_count * vertex_buffers[i].elements_count);
...
rf_extern rf_render_batch rf_create_default_render_batch_from_memory(rf_default_render_batch* memory)
{
    if (!memory) {
        return (rf_render_batch) {0};
    }
